...
class BindVisitor(NodeVisitor):
    ...
    def _visit_Instance_body(self, node, nodename, arrayindex=None):
        #if node.module not in self.module:
    ...
    def _case(self, comp, caselist, myframes):
        ...
        for i in range(1,len(caselist)):
            if caselist[i].statement is not None: self.visit(caselist[i].statement)
    ...
    def visit_Block(self, node):
        ...
        # stackNextFrame 返回下一个frame的scopechain，frametable里通过词典实现scopechain到frame的映�?        
    ...
    def makeDFTree(self, node, scope):
        ...
        if isinstance(node, Identifier):
            if node.scope is not None:
                ...
                return DFTerminal(name, lineno=node.linenoo)
            name = self.searchTerminal(node.name, scope)
            if name is None:
                raise verror.DefinitionError('No such signal: %s' % node.name)
            return DFTerminal(name, lineno=node.lineno)

        if isinstance(node, IntConst):
            return DFIntConst(node.value, lineno=node.lineno)

        if isinstance(node, FloatConst):
            return DFFloatConst(node.value, lineno=node.lineno)

        if isinstance(node, StringConst):
            return DFStringConst(node.value, lineno=node.lineno)
        ...
        if isinstance(node, UnaryOperator):
            ...
            return DFOperator((right_df,), node.__class__.__name__, lineno=node.lineno)
        ...
        if isinstance(node, Operator):
            ...
            return DFOperator((left_df, right_df,), node.__class__.__name__, lineno=node.lineno)
        ...
        if isinstance(node, Partselect):
            ...
            return DFPartselect(var_df, msb_df, lsb_df, lineno=node.lineno)
        ...
        if isinstance(node, Pointer):
            ...
            return DFPartselect(var_df, ptr_df, copy.deepcopy(ptr_df), lineno=node.lineno)
        ...
        if isinstance(node, Concat):
            ...
            return DFConcat(tuple(nextnodes), lineno=node.lineno)
    ...
    def setDataflow(self, dst, raw_tree, condlist, flowlist,
        ...
        for name, msb, lsb, ptr, part_msb, part_lsb in dst:
            #print('\n set data flow:')
            #print(f'type name:{type(name)},  msb:{msb}, type raw_tree:{type(raw_tree)}')
            #print('....')
